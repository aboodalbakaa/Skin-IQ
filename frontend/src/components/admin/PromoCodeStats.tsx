...
export function PromoCodeDetails({ code, orders }: PromoCodeStatsProps) {
            ...
            <div className="p-8 border-b border-border flex items-center justify-between bg-slate-50/50 dark:bg-white/5">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground font-medium">
                  Showing {orders.length} orders generated by this influencer code.
                </p>
              </div>
              <button 
                onClick={() => setIsOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                <div className="p-6 rounded-[2rem] bg-accent/5 border border-accent/10">
                  <p className="text-[10px] font-black uppercase tracking-widest text-accent/60 mb-2">Total Sales</p>
                ...
                <div className="p-6 rounded-[2rem] bg-emerald-500/5 border border-emerald-500/10">
                  ...
                  <p className="text-3xl font-black text-emerald-500">
                    {orders.reduce((s, o) => s + Number(o.total_amount), 0).toLocaleString()} <span className="text-xs">IQD</span>
                  </p>
                </div>
                <div className="p-6 rounded-[2rem] bg-blue-500/5 border border-blue-500/10">
                  <p className="text-[10px] font-black uppercase tracking-widest text-blue-500/60 mb-2">Avg. Order Value</p>
                  <p className="text-3xl font-black text-blue-500">
                    {(orders.reduce((s, o) => s + Number(o.total_amount), 0) / orders.length).toLocaleString(undefined, { maximumFractionDigits: 0 })} <span className="text-xs">IQD</span>
                  </p>
                </div>
              </div>
