  ...
  const handleCheckout = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (result.error) {
        ...
        setSuccess(true);
        if (result.orderId) setOrderId(result.orderId.slice(0, 8).toUpperCase());
        if (result.totalAmount) setConfirmedTotal(result.totalAmount);
        clearCart();
      }
    });
  };
      ...
      <div className="min-h-screen py-24 px-4 bg-background flex items-center justify-center">
        <div className="bg-white dark:bg-[#0D1518] rounded-3xl shadow-xl border border-border p-8 md:p-12 max-w-lg text-center">
          ...
          <p className="text-slate-500 mb-2">
            {t('success_message')}{' '}
            <span className="font-mono text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-sm">
              {orderId}
            </span>
          </p>
          {confirmedTotal > 0 && (
            <p className="text-slate-500 mb-2 font-medium">
              IQD {confirmedTotal.toLocaleString()}
            </p>
          )}
          <p className="text-slate-500 mb-6">{t('success_contact')}</p>
      ...
      <div className="min-h-screen py-24 px-4 bg-background">
        <div className="max-w-3xl mx-auto text-center">
          <div className="bg-white dark:bg-[#0D1518] rounded-2xl shadow-sm border border-border p-12">
            ...
            <h2 className="text-xl font-medium text-slate-900 dark:text-slate-100 mb-2">{t('cart_empty_title')}</h2>
            <p className="text-slate-500 mb-6">{t('cart_empty_message')}</p>
          ...
          <div className="lg:col-span-7">
            <div className="bg-white dark:bg-[#0D1518] rounded-3xl shadow-sm border border-border p-6 md:p-8">
              ...
              <form id="checkout-form" onSubmit={handleCheckout} className="space-y-5">
                ...
                <div className="pt-4">
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t('location_title')}</p>
                  {googleMapsLink ? (
                    ...
                    >
                      ...
                      {locationLoading ? t('location_fetching') : t('location_button')}
                    </button>
                  )}
                ...
                <div className="pt-4 border-t border-slate-100 dark:border-slate-800 mt-6">
                  ...
                  >
                    {isPending
                      ...
                      : `${t('place_order')} (IQD ${finalTotal.toLocaleString()})`}
                  </button>
                  <p className="text-center text-xs text-slate-500 mt-4">{t('terms_note')}</p>
                </div>
              </form>
            </div>
          </div>
              ...
              <div className="border-t border-slate-200 dark:border-slate-800 pt-4 space-y-3">
                ...
                <div className="py-2">
                  <div className="flex gap-2">
                    ...
                    >
                      {isVerifying ? <Loader2 className="w-4 h-4 animate-spin" /> : t('promo_apply')}
                    </button>
                  </div>
                  {promoError && <p className="text-[10px] text-red-500 mt-1 font-medium">{promoError}</p>}
                  {promoType && (
                    <p className="text-[10px] text-emerald-500 mt-1 font-bold tracking-wider">
                      {promoType === 'percentage'
                        ? t('promo_saved_percent', { value: promoValue })
                        : t('promo_saved_fixed', { value: promoValue.toLocaleString() })}
                    </p>
                  )}
                </div>
                ...
                {discountAmount > 0 && (
                  <div className="flex justify-between text-sm text-emerald-600 dark:text-emerald-400 font-medium">
                    <span>Discount ({promoType === 'percentage' ? `${promoValue}%` : t('discount_fixed')})</span>
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="text-emerald-500 font-bold uppercase tracking-widest text-[10px] bg-emerald-50 dark:bg-emerald-900/20 px-2 py-1 rounded-md">
                    {t('delivery_value')}
                  </span>
                </div>
                <div className="flex justify-between items-end mb-1">
                  <span className="text-xs uppercase text-muted-foreground dark:text-slate-300 font-bold tracking-tighter">
                    {t('estimated_total')}
                  </span>
